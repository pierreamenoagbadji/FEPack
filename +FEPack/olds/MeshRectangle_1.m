...
classdef MeshRectangle < FEPack.meshes.Mesh
  ...
  methods
    ...
    function mesh = MeshRectangle(is_structured, BBx, BBy, numNodesX,...
      ...
      geomsg = ['Include "', FEPack.FEPackObject.pathCpp, '/+FEPack/+tools/FEPackGmsh.geo";\n\n',...
                'h0 = 0.1;\n',...
      ...
      % References to nodes and maps between edge nodes /////////////// %
      mesh.refPoints = zeros(mesh.numPoints, 1);                        %
      evec = [2; 3; 4; 5];                                              %
      Icoo = [1; 2; 1; 2];                                              %
      for idE = 1:4                                                     %
        pts = unique(mesh.segments(mesh.refSegments == evec(idE), :));  %
        mesh.refPoints(pts) = evec(idE);                                %
                                                                        %
        [~, cle] = sort(mesh.points(pts, Icoo(idE)));                   %
        mesh.maps{idE} = pts(cle);                                       %
      end                                                               %
      mesh.refPoints(mesh.corners) = 1;                                 %
      % /////////////////////////////////////////////////////////////// %

      mesh.domains{1} = FEPack.meshes.FEDomain(mesh, 'corners', 0, 1);
      for idom = 2:5
        mesh.domains{idom} = FEPack.meshes.FEDomain(mesh, side_names{idom-1}, 1, idom, mesh.maps{idom-1});
      end
      mesh.domains{6} = FEPack.meshes.FEDomain(mesh, 'volumic', 2, 0);
