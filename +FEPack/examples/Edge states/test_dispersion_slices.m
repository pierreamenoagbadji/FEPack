...
numDeltas = 100;
deltaMAX = 10;
deltas = linspace(0, deltaMAX, numDeltas);
...
%     % trisurf(meshXY.triangles, X, Y, Vpot([X, Y]));
%     trisurf(meshXY.triangles, X, Y, transVpot([X, Y])); 
%     hold on;
%     shading interp;
%     view(2);
...
mat_Vpot_u_v    = FEPack.pdes.Form.intg(cellXY, (transVpot * u) * v);
mat_Wpot_u_v    = FEPack.pdes.Form.intg(cellXY, (transWpot * u) * v);
...
for idP = 1:numDeltas
  ...
  parfor idI = 1:numLambda
    ...
    AA = mat_gradu_gradv +...
      ...
      + mat_Vpot_u_v + delta * mat_Wpot_u_v;
  ...
  %%
  fid = fopen(['outputs/slice_dispersion_', num2str(idP, '%0.3d'), '.txt'], 'w+');
  % fprintf(fid, 'lambda\t');
  % for idJ = 1:numEVP
...
system('tar -cvf slice_dispersion.tar -C ./outputs .');
